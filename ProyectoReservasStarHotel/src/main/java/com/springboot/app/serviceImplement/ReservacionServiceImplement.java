...
public class ReservacionServiceImplement implements ReservacionService {
    ...
    public ResponseEntity<Map<String, Object>> agregarReservacion(Reservacion reservacion) {
        Map<String, Object> respuesta = new HashMap<>();
        Huesped huespedExistente = huespedRepository.findByEmail(reservacion.getHuesped().getEmail());

        if (huespedExistente == null) {
            Huesped nuevoHuesped = reservacion.getHuesped();
            huespedExistente = huespedRepository.save(nuevoHuesped);
        }
        reservacion.setHuesped(huespedExistente);
