...
export class ReservacionesComponent implements OnInit {
  ...
  obtenerReservacionPorId(id: any) {
    ...
    this._reservacionesService.obtenerReservacionPorId(id).subscribe(
      (response) => {
        if (response) {
          form.controls['fechaIni'].setValue(
            response.reservacion.fechaIni || null
          );
          form.controls['fechaFin'].setValue(
            response.reservacion.fechaFin || null
          );
          form.controls['totalPago'].setValue(
            response.reservacion.totalPago || null
          );
          form.controls['estadoReservacion'].setValue(
            response.reservacion.estadoReservacion || null
          );
          form.controls['huesped']
            .get('idHuesped')
            ?.setValue(response.reservacion.huesped?.idHuesped || null);
          form.controls['habitacion']
            .get('idHabitacion')
            ?.setValue(response.reservacion.habitacion?.idHabitacion || null);
          form.controls['servicio']
            .get('id_servicio')
            ?.setValue(response.reservacion.servicio?.id_servicio || null);
        } else {
