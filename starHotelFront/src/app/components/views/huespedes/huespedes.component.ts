...
export class HuespedesComponent implements OnInit {
  ...
  guardar(formulario: any): void {
    if (this.formHuespedes.valid) {
      this._huespedesService.registrarHuesped(formulario).subscribe(
        (response) => {
          this.cerrarModal();
          this.obtenerHuespedes();
          this.resetForm();
  ...
  obtenerHuespedPorId(id: any) {
    ...
    this._huespedesService.listarHuespedPorId(id).subscribe(
      (response) => {
        if (response) {
          ...
          form.controls['nombreuser'].setValue(
            response.huesped.nombreuser || null
          );
          form.controls['telefono'].setValue(response.huesped.telefono || null);
          form.controls['direccion'].setValue(
            response.huesped.direccion || null
          );
        } else {
      ...
      if (result.isConfirmed) {
        this._huespedesService.eliminarHuesped(id).subscribe(
          (response) => {
            Swal.fire(
              'Eliminado!',
              'El huesped ha sido eliminado con éxito.',
              'success'
            );
            this.obtenerHuespedes();
          },
  ...
  editar(id: number, formulario: any): void {
    if (this.formHuespedes.valid) {
      this._huespedesService.editarHuesped(id, formulario).subscribe(
        (response) => {
          this.cerrarModal();
          this.obtenerHuespedes();
          this.resetForm();
